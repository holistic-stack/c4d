...
fn convert_node(node: &GeometryNode) -> Result<Manifold, Vec<Diagnostic>> {
    match node {
        ...
        GeometryNode::LinearExtrude { .. } => {
             // Placeholder for now, implemented in next steps but added to IR
             Err(vec![Diagnostic::error("linear_extrude not yet implemented in manifold-rs".to_string(), Span::new(0, 0).unwrap())])
        }
        GeometryNode::RotateExtrude { .. } => {
             // Placeholder for now
             Err(vec![Diagnostic::error("rotate_extrude not yet implemented in manifold-rs".to_string(), Span::new(0, 0).unwrap())])
        }
        ...
        GeometryNode::Resize { new_size, auto, child, span: _ } => {
            let mut m = convert_node(child)?;
            // Manifold::resize logic needs array, but IR has Vec<bool>.
        ...
        GeometryNode::Color { color, child, span: _ } => {
            ...
            // Apply color. If we merge multiple nodes, we assume simple propagation for now.
...
mod tests {
    ...
    fn test_circle_generation() {
        // FIXME: Parameter $fn seems ignored in some contexts or defaults are tricky.
        // Assuming default 30 segments for now.
